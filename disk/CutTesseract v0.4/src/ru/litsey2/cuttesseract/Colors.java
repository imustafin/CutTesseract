package ru.litsey2.cuttesseract;

import java.awt.Color;

/**
 * This class contains <code>Color</code> constants.
 * 
 * @author Ilgiz Mustafin
 */
public class Colors {
	/**
	 * The color of the X-axis
	 */
	public static final Color X_COLOR = Color.RED;
	/**
	 * The color of the Y-axis
	 */
	public static final Color Y_COLOR = Color.GREEN;
	/**
	 * The color of the Z-axis
	 */
	public static final Color Z_COLOR = Color.BLUE;
	/**
	 * The color of the W-axis
	 */
	public static final Color W_COLOR = Color.ORANGE;
	/**
	 * The color of the cube
	 */
	public static final Color CUBE_COLOR = Color.WHITE;
	/**
	 * The color of the resulting cut
	 */
	public static final Color CUT_COLOR = Color.MAGENTA;
	/**
	 * The color of the normal of the cut plane
	 */
	public static final Color NORMAL_COLOR = Color.YELLOW;

	/**
	 * The background color
	 */
	public static final Color BACKGROUND_COLOR = Color.BLACK;
}
